 ...
 * Expected $args:
 * - title: string
 * - faqs: array of FAQ arrays
 *   - question: string
 *   - answer: string
 *   - cta: array (optional: text, url, target)
 *   - open: bool (default false)
 */
$title = $args['title'] ?? 'Driver FAQ';
$faqs  = $args['faqs'] ?? [];

if ( empty( $faqs ) ) {
	$faqs = [
		[
			'question' => 'What is SilverRide?',
			'answer'   => 'Test answer 1',
			'open'     => true,
		],
		[
			'question' => 'Who books the rides?',
		...
		[
			'question' => 'Where can I go with my questions?',
			'answer'   => 'Email Onboarding@SilverRide.com. A real person will respond.',
			'cta'      => [
