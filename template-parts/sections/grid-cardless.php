<?php
/**
 * Grid Cardless Section Template
 * Matches screenshots/grid-cardless.png — Whitepapers layout
 *
 * White section, centered title + optional subtitle, 2-5 column cardless grid.
 * Each item: full-bleed photo on top, small meta (subtitle) line, bold title, muted description, pill CTA.
 * No card container — items sit directly on section background (cardless).
 *
 * ACF fields: title, description, grid_size, items[], background_color, footer_description, cta, hide_on, margin
 *
 * Items repeater: image, subtitle (used as meta/byline), title, description, link
 */
...
// Background — default white for cardless editorial look.
if ( $background_color ) {
...
// Grid columns — cardless defaults to 2 (whitepapers), supports 2-5.
$grid_map = [
	2 => 'md:grid-cols-2',
...
$grid_cols = $grid_map[ $grid_size ] ?? $grid_map[2];
...
<section
	...
	class="grid-cardless <?php echo esc_attr( trim( $margin_class . ' ' . $hide_class ) ); ?> py-16 md:py-20 <?php echo esc_attr( $bg_class ); ?>"
	...
	<div class="container mx-auto px-4 md:px-6 lg:px-8">
		<?php if ( $title || $description ) : ?>
			<div class="text-center max-w-4xl mx-auto mb-10 md:mb-12">
				<?php if ( $title ) : ?>
					<h2 class="text-3xl md:text-4xl lg:text-[2.375rem] font-bold text-[#1B1B1B] leading-tight">
						<?php echo wp_kses_post( $title ); ?>
					</h2>
				<?php endif; ?>
				<?php if ( $description ) : ?>
					<div class="mt-3 text-[15px] md:text-base leading-relaxed text-[#616161] max-w-3xl mx-auto prose prose-p:my-0 [&_p]:my-0 text-center">
		...
		<?php if ( ! empty( $items ) ) : ?>
			<div class="grid grid-cols-1 <?php echo esc_attr( $grid_cols ); ?> gap-8 lg:gap-10">
				...
				?>
					<article class="grid-cardless__item flex flex-col">
						<?php if ( ! empty( $image_url ) ) : ?>
							<div class="w-full overflow-hidden">
								<?php if ( $has_link ) : ?>
									<a href="<?php echo esc_url( $item_link['url'] ); ?>" target="<?php echo ! empty( $item_link['target'] ) ? esc_attr( $item_link['target'] ) : '_self'; ?>" class="block">
										<img
						...
						<div class="flex flex-col flex-1 pt-5">
							<?php if ( $item_subtitle ) : ?>
								<p class="text-base leading-relaxed text-[#2E4A7A]">
									<?php echo esc_html( $item_subtitle ); ?>
								</p>
							<?php endif; ?>

							<?php if ( $item_title ) : ?>
								<h3 class="mt-3 text-lg md:text-2xl font-bold text-[#1B1B1B] leading-snug">
									<?php if ( $has_link ) : ?>
							...
							<?php if ( $item_description ) : ?>
								<div class="mt-3 text-[14px] leading-[1.65] text-[#616161] prose prose-p:my-0 [&_p]:my-0">
