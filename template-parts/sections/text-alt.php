...
<div <?php echo $section_id; ?> class="<?php echo esc_attr($container_class); ?> <?php echo esc_attr($margin_top); ?> <?php echo esc_attr($margin_bottom); ?>" data-section-id="<?php echo esc_attr($template_part_name); ?>">
    <?php if(!empty($title)) : ?>
        <h2 class="text-3xl md:text-[2.5rem] font-semibold text-center capitalize tracking-wide !leading-snug mb-6">
            <?php echo wp_kses_post($title); ?>
        </h2>
    <?php endif; ?>
    
    <?php if ($description) : ?>
        <div class="prose text-lg leading-relaxed border-primary border-2 p-8 rounded-2xl">
